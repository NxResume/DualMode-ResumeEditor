...
<template>
  <div class="space-y-2">
    <h2 class="text-2xl text-gray-900 font-extrabold mb-4 text-center">
      {{ $t('auth.signIn') }}
    </h2>
    <Input
      v-model="email"
      type="email"
      class="px-4 py-2 border rounded w-full focus:outline-none focus:ring-2 focus:ring-gray-500"
      :placeholder="$t('auth.emailPlaceholder')"
    />
    <button
      class="text-white mb-2 py-2 rounded bg-gray-500 w-full transition hover:bg-gray-700"
    ...
    <div class="my-4 flex items-center">
      ...
      <span class="text-xs text-gray-400 mx-2">{{ $t('auth.or') }}</span>
      <div class="border-t border-gray-300 flex-grow" />
    </div>
    ...
    <div class="mt-4 text-center">
      <p class="text-sm text-gray-600">
        {{ $t('auth.bySigningIn') }}
        <a href="/terms" class="text-gray-900 font-medium hover:text-gray-700">
          {{ $t('auth.termsOfService') }}
        </a>
        {{ $t('auth.and') }}
        <a href="/privacy" class="text-gray-900 font-medium hover:text-gray-700">
